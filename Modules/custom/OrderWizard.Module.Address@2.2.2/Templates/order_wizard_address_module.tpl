...
<div class="order-wizard-address-module">
	<!--
	<div class="order-wizard-address-module-show-addresses-container order-wizard-address-module-address-list {{#if showAddress}} order-wizard-address-module-show {{else}} order-wizard-address-module-hide {{/if}}">
	-->
	<div class="order-wizard-address-module-show-addresses-container order-wizard-address-module-address-list">
		{{#if showAddress}}
			{{#if title}}
			...
			{{else}}
			<h3 class="order-wizard-address-module-title">
				{{translate 'Choose Shipping Address'}}
			</h3>
				<p class="shipping-mgs-title">{{translate 'Please select the final destination of your wine, regardless of shipping type.'}}</p>	
			{{/if}}
		{{/if}}
		{{#if isSameAsEnabled}}
		...
		{{#if showSingleAddressDetails}}
			{{#if showAddress}} <div data-view="Single.Address.Details" class="order-wizard-address-module-single"></div> {{/if}}
		{{else}}
			{{#if showAddressList}}

	...
	)
{
	...
	,	changeShippingType: function (e)
		{
			...
			if(shippingType == '3'){
				...
				jQuery('.order-wizard-step-button-continue').removeClass('will_continueBtn');
				jQuery('.order-wizard-step-button-continue').removeClass('rest_styles');
				jQuery('.order-wizard-step-button-continue').addClass('store_styles');
				//jQuery('order-wizard-step-button-container').css('text-align','initial');
			}else if(shippingType == '1'){
				jQuery('.order-wizard-step-button-continue').addClass('will_continueBtn');
				///jQuery('order-wizard-step-button-container').css('text-align','center');
				jQuery('.order-wizard-step-button-continue').removeClass('rest_styles');
				jQuery('.order-wizard-step-button-continue').removeClass('store_styles');
			}else{
				jQuery('.order-wizard-step-content-wrapper .order-wizard-step-actions').children('.third-party-div').remove();
				jQuery('.order-wizard-step-content-wrapper .order-wizard-step-actions').children('.div-third').remove();
				jQuery('.order-wizard-step-button-continue').addClass('rest_styles');
				jQuery('.order-wizard-step-button-continue').removeClass('store_styles');
				jQuery('.order-wizard-step-button-continue').removeClass('will_continueBtn');
				//jQuery('order-wizard-step-button-container').css('text-align','initial');
			}
			...
			{
				this.unsetAddressShippingType(true, shippingType);
			...
			this.enableDisableAddress(shippingType);
	...
	,	enableDisableAddress: function ()
		{
			var shippingType = jQuery("#custbody_vf_shippingtypeonso").val()
			,	collection = this.getAddressCollection()
			,	currentAddessId = jQuery(".address-details-container").attr("data-id");
			//console.log(currentAddessId)
			if(shippingType == '2'){
        if(collection.models){
					this.setAddress(currentAddessId);
					this.render();
					this.trigger('ready', true);
				}
				var user_zipcodes = [];
				_.each(collection.models, function(address){
					// Enable / Disabled 'Select Address' button
					if( address.get('country') != 'US' ){ jQuery('#address-details-'+address.get('internalid')).attr("disabled", true); }
					else { jQuery('#address-details-'+address.get('internalid')).attr("disabled", false); }
				});
       			 jQuery(".order-wizard-address-module .order-wizard-address-module-address-list").show();
				jQuery('.order-wizard-step-content-wrapper .order-wizard-step-actions .third-party-message').hide()
        ...
        if(collection.models){
				...
				_.each(collection.models, function(address){
					// Get all zip codes of user's addresses
					if( !_.isNull(address.get('zip')) )
						user_zipcodes.push( address.get('zip') );
				});
				...
				jQuery.ajax({
					...
					data: {action: 'localzipcodes', user_zipcodes: JSON.stringify(user_zipcodes)},
					method: 'POST',
					async: false,
					success: function ( res ){ 
						if(res!=null){
							for(var z=0; res!=null && z<res.length; z++)
								local_zipcode.push(res[z].zipcode);
						} 
					}
				});

				_.each(collection.models, function(address){
					if( local_zipcode.indexOf( address.get('zip') ) == -1 ){ 
						jQuery('#address-details-'+address.get('internalid')).attr("disabled", true);
					}
					else { 
						jQuery('#address-details-'+address.get('internalid')).attr("disabled", false); 
					}
				});
  	jQuery('.order-wizard-step-content-wrapper .order-wizard-step-actions .third-party-message').hide()
			}
				...
				jQuery(".order-wizard-address-module .order-wizard-address-module-address-list").show();
				jQuery('.order-wizard-step-content-wrapper .order-wizard-step-actions .third-party-message').show();
						}
...
jQuery('.order-wizard-step-button-continue').removeClass('will_continueBtn');
    ...
    var options = this.wizard.model.get('options'); 
			...
			this.wizard.model.set({
				options: _.extend(options, {
					'custbody_vf_shippingtypeonso': ''
				})
			});
			jQuery("#custbody_vf_shippingtypeonso").children().remove();
			jQuery("#custbody_vf_shippingtypeonso").append('<option value=" "></option>');
			jQuery(".order-wizard-address-module-shipping-type").hide();
			jQuery('.third-party-div').hide();
			jQuery('.order-wizard-shipmethod-module  .order-wizard-shipmethod-module-show ').hide();
	...
	,	isValid: function ()
		{
			...
			*/
			// DEV - Shipping Type validation
			//if(!this.model.get('options').custbody_vf_shippingtypeonso)
		
		var addresses = this.getAddressCollection()
		,	selected_address = addresses && addresses.get(this.model.get(this.manage));

		if( !this.model.get('options').custbody_vf_shippingtypeonso && this.options.show_shipping_type )
			{
				if(!selected_address)
				return jQuery.Deferred().reject(this.selectAddressErrorMessage);
				else
				return jQuery.Deferred().reject(this.selectShippingTypeErrorMessage);
			}
			else
			{
				//Is Active is override by child modules, like Shipping to hide this module in Multi Ship To
	...
	,	markSameAs : function (is_checked)
		{
			...
			// DEV - Hide shipping type & shipping options fields after checking the same as checkbox
			jQuery('.order-wizard-address-module-shipping-type').hide();
			jQuery('.address-details-actions').hide();

			// Check shipping type if user change the address
			var	ship_type = jQuery("#custbody_vf_shippingtypeonso").val() ? jQuery("#custbody_vf_shippingtypeonso").val() : this.wizard.model.get('options').custbody_vf_shippingtypeonso;
			
			this.enableDisableAddress(ship_type);
		}
	...
	,	getAddressListOptions: function ()
		{
			var shippingType = this.wizard.model.get('options').custbody_vf_shippingtypeonso;
		...
		,	'Address.List': function ()
			{
				...
				,	shippingType: this.wizard.model.get('options').custbody_vf_shippingtypeonso
				});
			}
		}
	...
	,	getContext: function ()
		{
			...
			var shipping_types = [];
            var configuration_Dom_Values = new Array();
			var configuration_Thr_Values = new Array();
			var configuration_Loc_Values = new Array();
			var configuration_Int_Values = new Array();
			var configuration_Sto_Values = new Array();
			var config_data_domestic= Configuration.get('Domestic');   // getting the data from the configuration record
			var config_data_third= Configuration.get('Third');   // getting the data from the configuration record
			var config_data_local= Configuration.get('Local');   // getting the data from the configuration record
			var config_data_inter= Configuration.get('International');   // getting the data from the configuration record
			var config_data_storage= Configuration.get('storage');   // getting the data from the configuration record
			var showLocalAndCall = false
			,	showStoreType = false
			,	showDomestic = false
			,	showThird = false
			,	showInternaltional = false
			,	showShippingType;
			_.each(config_data_domestic,function(value,index){
				configuration_Dom_Values[configuration_Dom_Values.length] = value.code;
			});
			_.each(config_data_third,function(value,index){
				configuration_Thr_Values[configuration_Thr_Values.length] = value.code;
			});
			_.each(config_data_local,function(value,index){
				configuration_Loc_Values[configuration_Loc_Values.length] = value.toLowerCase();
			});
			_.each(config_data_inter,function(value,index){
				configuration_Int_Values[configuration_Int_Values.length] = value.code;
			});
			_.each(config_data_storage,function(value,index){
				configuration_Sto_Values[configuration_Sto_Values.length] = value.code;
			});

			//console.log('configuration_Sto_Values',configuration_Sto_Values)
			/*console.log('configuration_Dom_Values',configuration_Dom_Values)
			console.log('configuration_Thr_Values',configuration_Thr_Values)
			console.log('configuration_Loc_Values',configuration_Loc_Values)
			console.log('configuration_Int_Values',configuration_Int_Values)
			console.log('configuration_Sto_Values',configuration_Sto_Values)*/
			//console.log("this.address.get('state')",this.address.get('state'))
			//console.log("this.address.get('country')",this.address.get('country'))
			//console.log('city',this.address.get('city'))
			var local_compare_city = this.address.get('city')
			,   lowercase_city = ''
			if(local_compare_city)
				lowercase_city = local_compare_city.toLowerCase()

			//console.log(configuration_Loc_Values)

			var ifStoreageState = _.contains(configuration_Sto_Values, this.address.get('state'))
			var ifDomestic = _.contains(configuration_Dom_Values, this.address.get('state'))
			var ifThirdParty = _.contains(configuration_Thr_Values, this.address.get('state'))
			var ifInternational = _.contains(configuration_Int_Values, this.address.get('country'));
			var ifLocalAndCall = _.contains(configuration_Loc_Values, lowercase_city);
			//console.log('ifStoreageState',ifStoreageState)
			if(this.address.get('country') == "US"){
				//showStoreType = true;
				if(this.address.get('state') ==  "CA"){ // Code for Local delivery and Will Call pick up
					if(ifLocalAndCall){
						showLocalAndCall = true;
					}
					if(ifDomestic){      /// Domestic shipping
						showDomestic = true;
					}
					showStoreType = true;
				}else if(ifStoreageState){     //Storage
					showStoreType = true;
				}else if(ifThirdParty){         // Third PParty
					showThird = true;
					showStoreType = true;
				}else if(ifDomestic){      /// Domestic shipping
					showDomestic = true;
					showStoreType = true;
				}
			}else if(ifInternational){ // code for International
				showInternaltional = true;
				showStoreType = true;
			}else{
				showStoreType = true;
			}

			if(!_.isUndefined(this.model.__customFieldsMetadata) && this.address.id)
			{
				for(var i=0; this.model.__customFieldsMetadata.custbody_vf_shippingtypeonso.options != null && i < this.model.__customFieldsMetadata.custbody_vf_shippingtypeonso.options.length; i++)
				{
					var shipTypeOptions = this.model.__customFieldsMetadata.custbody_vf_shippingtypeonso.options
					,	id = shipTypeOptions[i].id
					,	text = shipTypeOptions[i].text
					,	isActive = (shipTypeOptions[i].id == ship_type);
					
					if(showLocalAndCall && (id == 1 || id == 6)){
						shipping_types.push({id: id, text: text, isActive: isActive});
					}else if(showStoreType && (id == 1 || id == 3)){
						shipping_types.push({id: id, text: text, isActive: isActive});
					}else if(showDomestic && id == 2){
						shipping_types.push({id: id, text: text, isActive: isActive});
					}else if(showThird && id == 4){
						shipping_types.push({id: id, text: text, isActive: isActive});
					}else if(showInternaltional && id == 5){
						shipping_types.push({id: id, text: text, isActive: isActive});
					}
				}	
				showShippingType = this.options.show_shipping_type;
			...
			,	sameAsMessage: this.sameAsMessage
				//@property {Boolean} showSingleAddressDetails (DEV123)
			...
			,	showSaveButton: this.showSaveButton
				//@property {String} showAddress
			//,	showAddress: ( Number(ship_type) == 2 || Number(ship_type) == 5 || Number(ship_type) == 6 || Number(ship_type) == '' ) ? true : false
			,	showAddress: true
				//@property {String} showShippingType

	...
	,	getContext: function()
		{
			...
			// Replace 'Sold By' option value with 'Wine Source'
			var line_item = this.line_model.get('item');
			/*
			if(this.model.get('itemOptionId') == 'custitemvf_sold_by' && this.options.wineSource)
			{
				selected_value.label = this.options.wineSource || selected_value.internalid;
			}
			*/
			
			if(this.model.get('itemOptionId') == 'custitemvf_sold_by' && line_item.get('custitem_wine_source'))
			{
				selected_value.label = line_item.get('custitem_wine_source') || selected_value.internalid;
			}
			...
			,	isSoldBy: (this.model.get('itemOptionId') === 'custitemvf_sold_by')
				// @property {Boolean} isShipPickDate
			,	isShipPickDate: ( this.model.get('cartOptionId') === 'custcol_vf_estshipdate' )
				// @property {String} itemInternalId
			,	itemInternalId: ( !_.isUndefined(this.line_model) ) ? this.line_model.get('internalid') : ''
			};
			// @class Transaction.Line.Views.Option.View
		}
	});

	...
	)
{
	...
	,	events: {
		...
		// ,	'change input.sliderValue': 'updateRangeValues'
	...
	,	initialize: function ()
		{		
			...
			//this.on('afterViewRender', this.appendingFacetCount, this);
		}
...
*/	
	...
	,	renderFacets: function ()
		{
			if(this.facetId==='Source'){				
				this.$('.facets-faceted-navigation-item-facet-group').addClass('hide')
			}
			if(this.facetId==='Sold-Out'){				
				this.$('.facets-faceted-navigation-item-facet-group').addClass('hide')
			}
			if(this.facetId==='Out-of-Stock'){				
				this.$('.facets-faceted-navigation-item-facet-group').addClass('hide')
			}
			if(this.facetId==='custitem_producer'){				
				this.$('.facets-faceted-navigation-item-facet-group').addClass('hide')
			}
	...
	,	updateRangeValues: function (e, slider)
		{	
			$('#lowInput').prop('disabled', false);
			$('#highInput').prop('disabled', false);

			$('#lowInput').on('change keyup', function() {
				var lowVal = $('#lowInput').val();
				$('span[data-range-indicator="start"]').html('$' + lowVal).end()
				slider.values.low = lowVal 
			  });
			  
			  $('#highInput').on('change keyup', function() {
				var highVal = $(this).val();
				$('span[data-range-indicator="end"]').html('$' + highVal).end()
				slider.values.high = highVal
			  });

			  var	start = slider.values.low.toFixed(0)
			  ,	end =  slider.values.high.toFixed(0)
	...
	// ,	updateRangeInput: function (e, slider)
	// {
	// 	var startNum = $('#lowInput')
	// 	,   endNum = $('#highInput')
	// 	,  slider = $('#slider');

	// 	startNum.val(slider.attr('data-low'));
	// 	endNum.val(slider.attr('data-high'));
	// 	$('span[data-range-indicator="start"]').html("$" + startNum).end()
	// 	$('span[data-range-indicator="end"]').html("$" + endNum);
	...
	,	updateRangeSelection: function (e, slider)
		{
			...
			else if (!facet_values || parseFloat(facet_values.from) !== slider_values.low || parseFloat(facet_values.to) !== slider_values.high)
			{
				...
				var fasetID = this.facetId;
				$("#apply").click(function(){
				Backbone.history.navigate(translator.cloneForFacetId(fasetID, {
					from: slider_values.low
				,	to: slider_values.high
				}).getUrl(), {trigger: true});
					console.log('apply')
				});
			}
		  ...
		  var facet_id = this.facetId
			...
			if (facet_config.behavior !== 'range')
			{
				...
				//	setTimeout(function() { 
					...
					{
						var commerceCategoryURL = translator.categoryUrl
						,   translatorFacets = translator.facets
						,   translaterFacetURL = new Array()
						,   AppliedFacetSearchAPIURL 
						,   searchAPI = 'http://www.vin-folio.com/api/items?'
						,   singleFacetValue
						,   searchTotal
						,   keywords = translator.getOptionValue('keywords')

						if(!_.isEmpty(translatorFacets))
						{
							_.each(translatorFacets, function(translatorFacet,index)
							{
								var appliedFacetURL = encodeURIComponent(translatorFacet.value)
								//console.log(decodeURIComponent(translatorFacet.value))
								//console.log(encodeURIComponent(translatorFacet.value))
								//console.log(appliedFacetURL)
								translaterFacetURL[translaterFacetURL.length] = ""+translatorFacet.id+"="+appliedFacetURL
							});	
						}

						if(commerceCategoryURL)
						{
							AppliedFacetSearchAPIURL = searchAPI+"commercecategoryurl="+commerceCategoryURL
							if(!_.isEmpty(translatorFacets))
							{
							AppliedFacetSearchAPIURL = AppliedFacetSearchAPIURL+"&"+translaterFacetURL.join('&')	
							//console.log(translatorFacets)	
							}
						}
						else{
							if(!_.isEmpty(translatorFacets))
							{
							AppliedFacetSearchAPIURL = searchAPI+translaterFacetURL.join('&')	
							}					
						}

						singleFacetValue = facet_id+"="+encodeURIComponent(value.url);										
						
						if(keywords != null && !_.isEmpty(keywords))
						{
							finalSearchAPICall = searchAPI+"&"+singleFacetValue+"&q="+keywords
						}else
						{
							finalSearchAPICall = AppliedFacetSearchAPIURL+"&"+singleFacetValue
						}
						
							jQuery.ajax(
								{
								type:"GET",
								url:finalSearchAPICall,					
								//async:true,
								success: function(result){									
										$('[data-value="'+value.displayName+'"]').text(result.total)									
								}
								})	
	...
	,	getContext: function ()
		{
			...
			if (facet_config.behavior !== 'range')
			{
				...
				_.each(original_values, function(value)
				{
					...
					if (value.url !== '')
					{
						/*if(facet_id !== 'Source' && facet_id !== 'custitem_producer' && facet_id !== 'Sold-Out')
						{
						
						facet_url= self.searchResponse(value.url);
						
						//console.log(facet_id)

				    	value.facetresultsurl = facet_url
						}*/
						...
						if(facet_id == 'Size')
						{
							var label = Number(value.label)
							if(label <= 999)
							{
								value.displayName = value.label+' ml';
							}
							else
							{
								value.displayName = (value.label/1000)+' L';
							}	
						}else
						{
				...
				//console.log(facet_id)
				//console.log(facet_urlArr)

				/*if(!_.isEmpty(facet_urlArr))
				{
					jQuery.ajax({
					url: "/app/site/hosting/scriptlet.nl?script=customscript_webstore_search_api_res&deploy=1&urlarray="+facet_urlArr,
					data: '',
					async: false
					}).done(function() {
	  					console.log('success')
			...
			else //if (facet_config.behavior === 'range')
			{
				...
				range_from_label = range_from_label.split(".")[0];
				range_to_label = range_to_label.split(".")[0];
				range_from_label = range_from_label.replace(',','')
				range_to_label = range_to_label.replace(',','')
			}
			...
			//,   appendFacetCount: this.appendingFacetCount()
			};

...
,	function (
	...
	,	FacetsShopCategoryCellView
	,	FacetsProducerCategoryCellView
	...
	)
{
	...
	,	events: {
		...
		,	'click [data-action="set-out-of-stock"]': 'toggleOtOfStock'
		}
	...
	,	initialize: function (options)
		{
			...
			this.isSource=this.translator.getFacetConfig('Source');
			this.isIncludeOutOfStock = this.translator.getFacetConfig('OutOfStock');
	...
	,	toggleOtOfStock: function (e)
		{
			var current_target = jQuery(e.currentTarget);
			var _include_outof_stock = this.childViews.getOutOfStockData.call( this, {facetId:'OutOfStock'} );
			
			// For include out of stock
			/*if ( jQuery('[name="include_outofstock"]').is(':checked') )
			{
				var obj = _.findWhere(_include_outof_stock, {label: 'true'})
				if( !_.isUndefined(obj)){
					jQuery('[name="include_outofstock"]').prop('checked', true);
					Backbone.history.navigate(obj.url, {trigger: true});
				}
			}
			else if ( !jQuery('[name="include_outofstock"]').is(':checked') )
			{
				var page_url = Backbone.history.getFragment();
				if( page_url.indexOf('/OutOfStock/true') != -1)
					page_url = page_url.replace('/OutOfStock/true', '');
				jQuery('[name="include_outofstock"]').prop('checked', false);
				Backbone.history.navigate(page_url, {trigger: true});
			}*/

			// For exclude out of stock
			if ( jQuery('[name="exclude_outofstock"]').is(':checked') )
			{
				var obj = _.findWhere(_include_outof_stock, {label: 'false'})
				if( !_.isUndefined(obj)){
					jQuery('[name="exclude_outofstock"]').prop('checked', true);
					Backbone.history.navigate(obj.url, {trigger: true});
				}
			}
			else if ( !jQuery('[name="exclude_outofstock"]').is(':checked') )
			{
				var page_url = Backbone.history.getFragment();
				if( page_url.indexOf('/OutOfStock/false') != -1)
					page_url = page_url.replace('/OutOfStock/false', '');
				jQuery('[name="exclude_outofstock"]').prop('checked', false);
	...
	,	childViews: {
			getSourceData:function(options){
				var vals=_.findWhere(this.model.get('facets'), {url: options.facetId});
				var selected_values = this.options.translator.getFacetValue(options.facetId) || []
				var translator = this.translator;

				/*
				return d = vals.values.map( item => {
					console.log(item, '11111111', this.translator);
					var label	= item.label
					var url		= this.translator.cloneForFacetId(options.facetId, item.url).getUrl();
					var isActive= _.contains(selected_values, item.url)
					return {label:label,isActive:isActive,url:url}
				})
				*/

				var d = '';
				return d = _.map(vals.values, function(item){
					var label	 = item.label+'<br> MARKETPLACE';
					var url		 = translator.cloneForFacetId(options.facetId, item.url).getUrl();
					var isActive = _.contains(selected_values, item.url)
					var sourceClass = '', toolTip = '';
					if(item.url == 'PRODUCER'){ 
						sourceClass = 'facets-facet-browse-source-producer';
						toolTip = 'Producer Marketplace wines come directly from the source. We work closely with the trade, including top producers, negociants, importers, and wholesalers to bring you the highest quality wines from around the globe';
					}
					else if(item.url == 'COLLECTOR'){ 
						sourceClass = 'facets-facet-browse-source-collector';
						toolTip = 'Collector Marketplace wines come from trusted collectors who are either liquidating or rebalancing their cellars, or taking advantage of profitable market opportunities. All of these wines are physically stored in our state of the art, temperature controlled fine wine storage facility in Napa and each bottle is professionally inspected by our team of professionals.';
					}

					return {label:label,isActive:isActive,url:url,sourceClass:sourceClass,toolTip:toolTip}
				})
				
			}

		,	getOutOfStockData: function(options){
				var vals=_.findWhere(this.model.get('facets'), {url: options.facetId});
				var selected_value = this.options.translator.getFacetValue(options.facetId) || false;
				var translator = this.translator;
				...
				return d = _.map(vals.values, function(item){
						var label	 = item.url
						var url		 = translator.cloneForFacetId(options.facetId, item.url).getUrl();
						var isActive = (selected_value === item.url) ? 'yes' : 'no'
						return {label:label,url:url,isActive:isActive}
				});
			}

		,	getSoldOut: function(options){
			//console.log(options)
		...
		,	getSelectedOutOfStockData: function(options){
				return this.options.translator.getFacetValue(options.facetId);
		...
		,	'Facets.Items': function()
			{
				...
				// Dev : Top 100 Collectible Wines Code
				if( !_.isUndefined(this.model.get('category')) && !_.isUndefined(this.model.get('category').get('urlfragment')) &&  this.model.get('category').get('urlfragment') == 'top-100-collectible-wines' )
				{
					display_option = {columns:4, icon:"icon-display-grid", id:"grid", isDefault:true, name:"Grid", template:"facets_item_cell_grid_top_100_collectible.tpl"}; 
				}
		...
		,	'Facets.ProducerCategoryCells': function()
			{
				return new FacetsProducerCategoryCellView({
					producer_sub_cat: this.model.get('category') ? this.model.get('category').get('categories') : []
				});
			}
		...
		,	'Facets.CategoryCells': function()
			{
				...
				,	childViewOptions:{parent_category_type:  this.model.get('category').get('custrecord_category_type')}
				});
			}

		,	'Facets.ShopCategoryCells': function()
			{
				return new BackboneCollectionView({
					childView: FacetsShopCategoryCellView
				,	collection: this.model.get('category') ? this.model.get('category').get('categories') : []
				,	childViewOptions:{parent_category_type:  this.model.get('category').get('custrecord_category_type')}
				});
			}
	...
	,	getContext: function()
		{
			...
			//get the selected facets to find out Size
				var selectedFacet = this.translator.cloneWithoutFacetId('category').getAllFacets()
				,   selectedFacetId
				,   selectedFacetValues;

				//loop through each facet and get the size facet values to pass it on to Facet.ItemCell.View
				_.each(selectedFacet, function(facet)
					{
						if(facet.id === 'Size')
						{
						  selectedFacetId = facet.id;		
						  selectedFacetValues = facet.value;
						}
					});	


					$.cookie('facetresultspage',true)
					if(selectedFacetId == 'Size')
					{			
					$.cookie('selectedfacetvalue',selectedFacetValues)
					//console.log($.cookie('selectedfacetvalue'))
					}

					if(_.isUndefined(selectedFacetId))
					{
						$.cookie('selectedfacetvalue','')	
					}	
					

			var category_type = 1;
			if( !_.isUndefined(this.model.get('category')) && !_.isUndefined(this.model.get('category').get('custrecord_category_type')) && this.model.get('category').get('custrecord_category_type') != '' )
			{
				category_type = this.model.get('category').get('custrecord_category_type');
			}

			var _source = this.childViews.getSourceData.call( this, {facetId:'Source'} );
			var selected_outof_stock = this.childViews.getSelectedOutOfStockData.call( this, {facetId:'OutOfStock'} )

			var sold_out_data = this.childViews.getSoldOut.call( this, {facetId:'Out-of-Stock'} )
			//console.log(sold_out_data)
				
			var find_soldout =_.findWhere(sold_out_data, {label: 'false'})
			,   soldoutActive = false
			//console.log(find_soldout)


			if(!_.isEmpty(find_soldout))
			{
				var soActive = find_soldout.isActive	
			...
			,	isSource:this.isSource
				// @property {Array} sourceFilter
			,	sourceFilter: this.isIncludeOutOfStock ? _source : []
				// @property {Boolean} isIncludeOutOfStock
			,	isIncludeOutOfStock: false // this.isIncludeOutOfStock
				// @property {Boolean} selectedOutOfStock
			,	selectedOutOfStock: selected_outof_stock

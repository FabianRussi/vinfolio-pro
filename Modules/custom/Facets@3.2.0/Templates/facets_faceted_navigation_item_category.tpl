...
{{#if showFacet}}
<div class="facets-faceted-navigation-item-category" style="display: none;"><!-- HIDE Category structure from LEFT PANEL -->
	{{#if isUncollapsible}}
		...
		<div class="{{#if isCollapsed}} collapse {{else}} collapse in {{/if}} facets-faceted-navigation-item-category-facet-group-wrapper">
			<div class="facets-faceted-navigation-item-category-facet-group-content">
				<ul class="facets-faceted-navigation-item-category-facet-optionlist">
					{{#each displayValues}}
						<li>
							<a class="facets-faceted-navigation-item-category-facet-option1 {{#if isActive}}option-active{{/if}}" href="{{#if externalLink}}{{externalLink}}{{else}}{{link}}{{/if}}" title="{{label}}" {{#if externalLink}}target="_blank"{{/if}}>
								{{displayName}}
							</a>
						</li>
					{{/each}}

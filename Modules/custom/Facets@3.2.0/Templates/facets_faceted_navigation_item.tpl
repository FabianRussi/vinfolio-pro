					...
					{{#each displayValues}}
						<li>
							<a class="facets-faceted-navigation-item-facet-option {{#if isActive}}option-active{{/if}}" href="{{link}}" title="{{label}}">
								{{#if ../isMultiSelect}}
									<label><input type="checkbox" class="facets-faceted-navigation-item-facet-multi" {{#if isActive}}checked {{/if}} /></label>
								{{/if}}

								<span>{{displayName}} <!-- (<span data-value="{{displayName}}"></span>) -->
									<!-- <script>
									//console.log("{{{facetresultsurl}}}")	
									jQuery.ajax({
										url: '{{{facetresultsurl}}}'
										/*success: function(result)
										{
											//console.log(result.total)
											$('[data-value="{{displayName}}"]').text(result.total)
										}*/
						...
						{{#each extraValues}}
							<li>
								<a class="facets-faceted-navigation-item-facet-option {{#if isActive}}option-active{{/if}}" href="{{link}}" title="{{label}}">
									...
									{{displayName}} <!-- (<span data-value="{{displayName}}"></span>) -->
									<!-- <script>
										//console.log("{{{facetresultsurl}}}")
										jQuery.ajax({
										url: '{{{facetresultsurl}}}'

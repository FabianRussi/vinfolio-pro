	...
	)
{
	...
	,	getContext: function ()
		{
			var shippingType = (!_.isUndefined(this.options.shippingType) && this.options.shippingType != '') ? this.options.shippingType : ''
			,	is_disabled = '';
			...
			/*if(shippingType == '2'){
				if( country != 'US' ){ is_disabled = true }
				...
				var local_zipcode = ['94117', '20521'];
				if( local_zipcode.indexOf( this.model.get('zip') ) == -1 ){ is_disabled = true }
				else { is_disabled = false }
			}

			console.log('shippingType: '+shippingType+' || is_disabled: '+is_disabled);

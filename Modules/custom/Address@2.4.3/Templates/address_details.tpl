...
<div class="address-details">
	<div class="address-details-container" data-id="{{internalid}}" {{#if isManageOptionsSpecified}} data-manage="{{manageOption}}" {{/if}}>
		...
		{{#if showSelectionButton}}
			<button class="address-details-select-address" data-action="select" data-id="{{internalid}}" id="address-details-{{internalid}}">
...
jQuery(document).ready(function(){
	...
	var addressId = {{internalid}};
	var country = '{{country}}';
	console.log("shippingType: "+shippingType+" || addressId: "+addressId+" || country: "+country);

	if(shippingType == '2' && (country == 'US' || country == 'United States')){
		jQuery('#address-details-'+addressId).hide();
	}
});
*/
</script>

	...
	)
{
	...
	,	events: {
		...
		//,	'change [data-action="set-ship-pick-date"]': 'updateShipDate'
		}
	...
	,	updateShipDate: function updateShipDate(e)
		{
			var self = this
			,	$line = null
			,	$form = jQuery(e.target).closest('form')
			,	options = $form.serializeObject()
			,	internalid = e.target.getAttribute('data-ship-internalid')
			,	line = this.model.get('lines').get(internalid)
			,	$input = jQuery('#quantity-'+internalid)
			,	isSet = false;
			...
			if (internalid)
			{
				...
				{
					var option = line.get('options').models[i];
					if(option.get('cartOptionId') == 'custcol_vf_estshipdate')
					{
						option.get('value').label = e.target.value;
						option.get('value').internalid = e.target.value;
						isSet = true;
					}
				}

				// Set 'Ship/Pick Date' in model
				/*var selected_value = this.model.get('value');
				this.model.set({
					selected_value: _.extend(selected_value, {
						'label': e.target.value, 'internalid': e.target.value
					})
				});*/
	...
	,	updateItemQuantity: function updateItemQuantity(e)
		{
			...
			if (!validInput || options.quantity)
			{
				...
				if (new_quantity !==  current_quantity)
				{
					...
					var invalid = line.validate();
					var err = [];
					if(case_size > 0 && Number(new_quantity % case_size) > 0)
					{
...
*/
			...
			if(is_loged_in)
			{
				...
				analytics.track("Product Removed",
					{"email":userEmail,'first_name': firstName,'last_name':  lastName,"products": cartArray});

...
<tr id="{{lineId}}" data-item-id="{{itemId}}" data-type="order-item" class="{{#if showGeneralClass}} {{generalClass}} {{/if}} cart-lines-row">
  <td class="cart-lines-table-middle">
		<div {{#if livExError}}class="cart-livexborder"{{/if}}>
			{{#if livExError}}
			<div class="cart-livex-error"><p>{{livExErrorMessage}}</p>
			<a href="#" data-touchpoint="home" data-hashtag="#shop-wine/originalwoodcase">See other offers of same wine</a></div>
			{{/if}}
     <div class="row" style="padding-top:10px">
      <div class="col-md-2">
        <div class="cart-lines-thumbnail"> {{#if isNavigable}} <a {{{linkAttributes}}}> <img src="{{resizeImage thumbnail.url 'tinythumb'}}" alt="{{thumbnail.altimagetext}}"> </a> {{else}} <img src="{{resizeImage thumbnail.url 'thumbnail'}}" alt="{{thumbnail.altimagetext}}"> {{/if}} </div>
      </div>
      <div class="col-md-4">
        <div class="cart-lines-name"> {{#if isNavigable}} <a {{{linkAttributes}}} class="cart-lines-name-link"> {{item._name}} </a> {{else}} <span class="cart-lines-name-viewonly">{{item._name}}</span> {{/if}} </div>
				{{#if isNotLivExItem}}
          ...
          {{line.item.custitem_vf_shipping_message}} </div>
      </div>
      <div class="col-md-2"> {{#if showSummaryView}}
        <div class="cart-lines-summary cart-lines-qty-amount" data-view="Item.Summary.View"></div>
        {{/if}}
        <div class="cartsummarycase">{{#if line.item.custitem_case_size}}Case <span>{{line.item.custitem_case_size}}</span>{{else}}<span style="visibility: hidden">0</span>{{/if}}</div>
        <br>
        <div data-view="Item.Tax.Info"></div>
        <br>
      </div>
      <div class="col-md-2 price-text-right mobile-hide">
        <div class="cart-lines-price">
          <div data-view="Item.Price"></div>
        </div>
      </div>
      <div class="col-md-2 price-text-right mobile-hide">
        <div class="cart-item-summary-item-list-actionable-amount"> {{#if isPriceEnabled}} <span class="cart-item-summary-amount-value">{{ line.total_formatted }}</span> {{/if}} </div>
      </div>
    </div>

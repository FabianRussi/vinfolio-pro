	...
	,	newsletterSubscribe: function newsletterSubscribe (e)
		{
			...
			if (promise)
			{
				...
				}).done (function ()
				{
					...
					var status = self.model.get('code');

						//Passing the user email when user subscription is successful
						if(status === 'OK')
						{
							var userEmail = self.model.get('email');
							$.cookie('homenewsletteruser',userEmail)
						}
	...
	,	getContext: function getContext ()
		{
			...
			var successCode = this.state.code
			,   successMsg = this.state.messageType

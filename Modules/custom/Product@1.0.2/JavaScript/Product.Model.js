	...
	)
{
	...
	var ProductModel = Backbone.Model.extend(
			...
			validation: {
				'quantity' : {
					fn: function quantityValidator (new_quantity)
					{
						...
						,   placeholder = jQuery('.quantity_availability_place_holder')

						,	soldbyVal		 = self.get('custcol_vf_soldby')
						,   product_quantity_available = jQuery('label[data-label-value="'+soldbyVal+'"]').attr('data-label-quantity-available')
						...
						//console.log(_.isNumber(product_quantity_available))
						if(product_quantity_available >= 1)
						this.setAvailableQuantity(current_quantity,product_quantity_available);
		...
		,   setAvailableQuantity: function setAvailableQuantity(current_quantity,product_quantity_available)
			{

				var placeholder = jQuery('.quantity_availability_place_holder');
		   			if(current_quantity < product_quantity_available || !_.isNumber(current_quantity) || _.isNaN(current_quantity) || current_quantity < 1){
		   			placeholder.empty();	
		   			}

				if(current_quantity > product_quantity_available)
				{
					placeholder.empty();
					this.set('quantity',product_quantity_available,{silent: false});
			    	var message = 'The quantity you selected exceeds available amount in inventory. The quantity has been adjusted to the highest available quantity.';  //Error message
					//Trying to return the warning from the GlobalViewsMessageView
					var global_view_message = new GlobalViewsMessageView({
							message: message
						,	type: 'warning'
						,	closable: false
					});
					//appending the message to the particular placeholder

	...
	,	addToCart: function (e)
		{
			...
			var	item				= selected_line.get('item')
			,	case_size			= Number(item.get('custitem_case_size')) > 0 ? Number(item.get('custitem_case_size')) : 0
			,	qty_available		= item.get('quantityavailable')
			,	scasaletype_text	= item.get('custitem_vf_scasaletype')
			,	scasale_type		= ''
			,	isSpecialOrderItem	= item.get('custitem_onlinewithoutpo')
			,	live_ex_qty			= _.isUndefined(item.get('custitem_livex_prearrival_qty')) ? 0 : item.get('custitem_livex_prearrival_qty')
			,	objSCASaleTypes		= viewSCASaleType.getCustomList()
			,	item_internal_id	= item.get('internalid')
			,	qty_in_cart			= 0
			,	added_in_cart		= false;
			...
			var inCartItem = _.find(lines, function (line)
							{
								if(item_internal_id == line.get('item').get('internalid'))
									return line;
							});
			if(inCartItem){
				qty_in_cart = inCartItem.get('quantity');
				added_in_cart = true;
			}
			...
			if( scasale_type == 1 || scasale_type == 4 || scasale_type == 5 || scasale_type == 8){
				custom_qty_allow = qty_available;
			}
			else if( scasale_type == 2 || scasale_type == 3 || scasale_type == 6 || scasale_type == 7 ){
			...
			var err = [];
			if(case_size > 0 && Number(quantity % case_size) > 0)
			{
				err.push('Add quantity only in multiples of '+case_size);
			}
			else if(case_size && custom_qty_allow != 99999 && quantity > custom_qty_allow){
				err.push('Please add '+custom_qty_allow+' or less quantity of this item');
			}
			else if((case_size == "" || case_size == null) && custom_qty_allow != 99999 && quantity > custom_qty_allow){
				err.push('Please add '+custom_qty_allow+' or less quantity of this item');
			}
			else if (added_in_cart && qty_in_cart >= custom_qty_allow)
			{
				err.push('You have exceeded the max quantity '+custom_qty_allow+' for this item');
			}
			else if (added_in_cart && custom_qty_allow)
			{
				var qty_remaining = Number(custom_qty_allow - qty_in_cart);
				
				if (quantity > qty_remaining)
					err.push('You can add only '+qty_remaining+' quantity for this item');
			}

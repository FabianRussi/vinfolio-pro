...
<div class="header-sidebar-wrapper">
	...
	<div class="header-sidebar-menu-wrapper" data-type="header-sidebar-menu">
		...
		<ul class="header-sidebar-menu">
			{{#each categories}}
				{{#if text}}
					<li class="{{#if @last}}header-sidebar-menu-lastoption{{/if}}">
						{{#if external_url}}
						<a class="{{class}}" href="{{external_url}}" {{#if categories}}data-action="push-menu"{{/if}} name="{{text}}">
							{{text}}
							{{#if categories}}<i class="header-sidebar-menu-push-icon"></i>{{/if}}
						</a>
						{{else}}
						<a class="{{class}}" {{objectToAtrributes this}} {{#if categories}}data-action="push-menu"{{/if}} name="{{text}}">
							{{text}}
							{{#if categories}}<i class="header-sidebar-menu-push-icon"></i>{{/if}}
						</a>
						{{/if}}
						...
						{{#if categories}}
							<ul>
								...
								<li>
									{{#if external_url}}
										<a class="{{class}}" href="{{external_url}}" {{#if categories}}data-action="push-menu"{{/if}}>
										{{text}}
										{{#if categories}}<i class="header-sidebar-menu-push-icon"></i>{{/if}}
										</a>
									{{else}}
									...
									<ul>
										...
										{{#if external_url}}
										<li>
											<a class="{{class}}" href="{{external_url}}" >{{text}}</a>
										</li>
										{{else}}
										<li>
											<a class="{{class}}" {{objectToAtrributes this}} name="{{text}}">{{text}}</a>
										</li>
										{{/if}}
										{{/each}}
									</ul>
			...
			{{/if}}
			<li data-view="QuickOrderHeaderLink" class="hide">
			</li>
			<li data-view="RequestQuoteWizardHeaderLink" class="hide">
			</li>
	...
	{{#if showLanguages}}
	<div data-view="Global.HostSelector" class="hide"></div>
	{{/if}}
	{{#if showCurrencies}}
	<div data-view="Global.CurrencySelector" class="hide"></div>
	{{/if}}

...
<section class="overview-home-mysettings">
	...
	<div class="overview-home-mysettings-row">
	<div class="overview-home-mysettings-collector">
		<article class="overview-profile">
			<div class="overview-profile-header">
				<h4>{{translate 'Collector Profile'}}</h4>
			</div>
			<section class="overview-profile-card">
				<div class="overview-profile-card-content">
					{{#if name}}
						<p class="overview-profile-name">{{name}}</p>
					{{else}}
				<div class="overview-profile-card-content">
					{{#if hasCollectorProfile}}
					<p>{{translate 'We have your wine preferences on file for this account.'}}</p>
					{{else}}
					<p>{{translate 'We have no wine preferences on file for this account.'}}</p>
					{{/if}}
				</div>	
				<a href="/collectorprofile" class="overview-profile-card-button-edit">{{translate 'Update'}}</a>
			{{/if}}
				</div>
			</section>
		</article>
	...
	<div class="overview-home-mysettings-referral">
		<article class="overview-profile">
			<div class="overview-profile-header">
				<h4>{{translate 'Refer a Friend'}}</h4>
			</div>
			<section class="overview-profile-card referral-content">
				<div class="overview-profile-card-content">
						<a href="/invite" class="gold-button">{{translate 'GET 10%'}}</a>
						<p>{{translate 'for each friend you refer.'}}</p>
						<span class="gold-line"></span>
				</div>

				<a href="/invite" class="overview-profile-card-button-edit">{{translate 'Learn more'}}</a>

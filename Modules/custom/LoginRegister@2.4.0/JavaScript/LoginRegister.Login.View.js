			...
			this.trackEvent(function ()
			{
				...
				else
				{
					...
					if (url_options.origin && touchpoints[url_options.origin])
					{
						...
						// DEV - Redirected to home page
						var url = touchpoints.home;
					...
					else
					{
						...
						var userId = response.user.internalid
						,   collector_profile_url


							jQuery.ajax({
								dataType: "json",
								url: "/app/site/hosting/scriptlet.nl?script=customscript_get_collector_profile&deploy=1",
								data: {userid: userId},
								method: 'POST',
								async: false,
								success: function ( res ){ 
									var collectorProfile = res.collectorprofile
									if(!_.isEmpty(collectorProfile))
									{
										collector_profile_url = touchpoints.home 
									}else
									{
										collector_profile_url = touchpoints.customercenter+'#/collectorprofile'
									}
								}
							});
							console.log(collector_profile_url)
							//debugger;
							// otherwise we need to take it to the customer center
							// window.location.href = touchpoints.customercenter;

							// DEV - Redirected to home page
							window.location.href = collector_profile_url;
						}
					}
				}
			});

    ...
    return SCModel.extend({
        ...
        update: function (data) {
            var collectorProfile = {
                'custentity_collector_profile_france': data.custentity_collector_profile_france,
                'custentity_collector_profile_italy': data.custentity_collector_profile_italy,
                'custentity_c_profile_newworld': data.custentity_c_profile_newworld,
                'custentity_c_profile_other': data.custentity_c_profile_other,
                'custentity_cp_burgundy': data.custentity_cp_burgundy,
                'custentity_cp_other_france': data.custentity_cp_other_france,
                'custentity_cp_other_new_world': data.custentity_cp_other_new_world
            };
            var jsonData = JSON.stringify(collectorProfile), headers = new Array(), responsebody = '', stSuiteletUrl = '';
            ...
            if (findNewsletterSub.subscribed == true || findNewsletterSub.subscribed == 'T') {
                var currentDate = new Date(), convertDate = currentDate.toISOString();
            ...
            if (order_fields.items && order_fields.items.length) {
                ...
                lines.forEach(function (line) {
                    ...
                    if (!line.item) {
                        ...
                        if (line.item.custitem_livex_order_guid) {
                            var custitem_livex_order_guid = line.item.custitem_livex_order_guid;
                            var data = { 'orderGUID': [custitem_livex_order_guid] };
                            var headers = {
                                'CLIENT_KEY': 'dbfb9fa7-1582-429d-8007-5c7dfa8a18ee',
                                'CLIENT_SECRET': '1890Bry',
                                'ACCEPT': 'application/json',
                                'CONTENT-TYPE': 'application/json'
                            };
                            var b = nlapiRequestURL('https://api.liv-ex.com/exchange/v4/orders', JSON.stringify(data), headers, 'POST');
                            line.livexresponse = b.getBody();
                        } else {
                            line.livexresponse = '';
                        }
                    }
        ...
        categoryColumns: {
            ...
            sideMenu: {
                ...
                'fields': [
                    ...
                    'sequencenumber',
                    'urlfragment',
                    'custrecord_external_url',
                    'custrecord_category_sort_option',
                    'custrecord_hide_in_navigation',
                    'custrecord_category_type',
                    'custrecord_region_under_producer',
                    'custrecord_producer_image_url',
                    'displayinsite',
        ...
        _addTransactionColumnFieldsToOptions: function (line) {
            ...
            _.each(lineFieldsId, function (field) {
                if (field.indexOf('custcol') === 0) {
                    ...
                    if (fieldValue !== null) {
                        line.options.push(self.transactionModelGetLineOptionBuilder(field, null, self.transactionModelGetLineOptionValueBuilder(undefined, fieldValue), null));
                    }
                }
            });
        }
